"""
Phase 4: Train multimodal late-fusion layer (text xlm-roberta + image resnet18 ai_vs_real)
on HF-derived paired data. Exports to trained_hugging_face_models/phase4
"""
import os
import json
import time
...
OUT = ROOT / "trained_hugging_face_models" / "phase4"
...
class TextEmbedder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        return self.backbone(input_ids=input_ids, attention_mask=attention_mask).last_hidden_state[:, 0, :]
...
class FusionDataset(Dataset):
    def __init__(self, df, tok, img_transform, max_len=128):
        self.rows = df.to_dict("records"); self.tok = tok
        self.img_transform = img_transform; self.max_len = max_len
    ...
    def __getitem__(self, idx):
        r = self.rows[idx]
        enc = self.tok(str(r["text"]), max_length=self.max_len, padding="max_length",
                       truncation=True, return_tensors="pt")
        img = self.img_transform(Image.open(r["image_path"]).convert("RGB")) if Path(r["image_path"]).exists() else torch.zeros(3, 224, 224)
        return {"input_ids": enc["input_ids"].squeeze(0),
                "attention_mask": enc["attention_mask"].squeeze(0),
                "image": img, "label": torch.tensor([float(r["toxic"])])}
...
def train():
    ...
    train_df = pd.read_csv(DATA / "phase4_train.csv")
    train_df = train_df.dropna(subset=["image_path"])
    train_ds = FusionDataset(train_df, tok, img_transform)
    train_loader = DataLoader(train_ds, batch_size=16, shuffle=True, num_workers=2)

    # val: phase2 val text + random ai_vs_real images
    val_df = pd.read_csv(DATA / "phase2_val.csv")
    e = ImageEmbedder().to(device).eval()
    vrows = []
    for _, row in val_df.iterrows():
        toxic = int(row["label"])
        cls_ = "ai" if toxic else "real"
        files = list((DATA / "images" / "ai_vs_real" / "val" / cls_).glob("*.jpg")) or \
                list((DATA / "images" / "ai_vs_real" / "train" / cls_).glob("*.jpg"))
        vrows.append({"text": row["text"], "toxic": toxic,
                      "image_path": str(np.random.choice(files))})
    val_ds = FusionDataset(pd.DataFrame(vrows), tok, img_transform)
    ...
    for epoch in range(epochs):
        ...
        for b in train_loader:
            ...
            img = b["image"].to(device); lbl = b["label"].to(device).squeeze(1)
            optimizer.zero_grad()
            with torch.no_grad():
                text_emb = embedder_text(ids, mask)
        ...
        embedder_image.eval(); fusion.eval(); correct = total = 0
        with torch.no_grad():
            for b in val_loader:
                ids = b["input_ids"].to(device); mask = b["attention_mask"].to(device)
                img = b["image"].to(device); lbl = b["label"].to(device).squeeze(1)
                text_emb = embedder_text(ids, mask)
                image_emb = embedder_image(img)
                preds = (torch.sigmoid(fusion(text_emb, image_emb)) > 0.5).float()
                correct += (preds == lbl).sum().item(); total += lbl.size(0)
        acc = correct / total
        ...
        if acc > best:
            ...
            torch.save({"fusion_state": fusion.state_dict(),
                        ...
                        "text_model": TEXT_MODEL, "val_acc": acc}, OUT / "fusion.pt")
    ...
    with open(OUT / "config.json", "w") as f:
        json.dump({"text_model": TEXT_MODEL, "text_dim": 768, "image_dim": 512,
                   ...
                   "data": "HF multilingual text + AI-vs-Real images"}, f, indent=2)
