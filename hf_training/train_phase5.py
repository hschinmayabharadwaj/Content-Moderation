"""
Phase 5: HITL decision layer + drift baseline from trained Phase 2 model.
Exports thresholds + drift stats to trained_hugging_face_models/phase5
"""
import os
import json
import time
...
OUT = ROOT / "trained_hugging_face_models" / "phase5"
DATA = ROOT / "hf_training" / "_data"
P2 = ROOT / "trained_hugging_face_models" / "phase2"
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        enc = self.tok(self.texts[idx], max_length=self.max_len, padding="max_length",
...
def main():
    ...
    with torch.no_grad():
        for b in loader:
            ...
            trues.extend(b["label"].squeeze(1).cpu().numpy().tolist())
    scores = np.array(scores); trues = np.array(trues)

    best, best_low, best_high = -1, 0.35, 0.65
    ...
    for lo in grid:
        for hi in grid:
            ...
            agree[(trues == 1) & (preds == 1)] = 0.5
            f = agree.mean()
            if f > best:
                best, best_low, best_high = f, lo, hi

    preds = np.full_like(scores, 1)
    preds[scores < best_low] = 0
    preds[scores > best_high] = 2
    tp = ((preds == 2) & (trues == 1)).sum(); fp = ((preds == 2) & (trues == 0)).sum()
    fn = ((preds != 2) & (trues == 1)).sum()
    prec = tp / (tp + fp + 1e-9); rec = tp / (tp + fn + 1e-9)
    f1 = 2 * prec * rec / (prec + rec + 1e-9)

    baseline = {"mean": float(scores.mean()), "std": float(scores.std()),
